  ...
  (
    ...
    return (
      <div className="relative w-full">
        {leftIcon && (
          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground [&_svg]:h-4 [&_svg]:w-4 [&_svg]:shrink-0 z-10">
            {leftIcon}
          </div>
        )}
        ...
        {(rightIcon || isPassword || showClearButton) && (
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 z-10">
            {rightIcon && <div className="text-muted-foreground [&_svg]:h-4 [&_svg]:w-4 [&_svg]:shrink-0">{rightIcon}</div>}
            {showClearButton && (
              <button type="button" onClick={handleClear} className="text-muted-foreground hover:text-foreground" tabIndex={-1}>
                <X />
              </button>
            )}
            {isPassword && (
              <button type="button" onClick={togglePasswordVisibility} className="text-muted-foreground hover:text-foreground" tabIndex={-1}>
